...
it('exposes all theme manifest values', function (): void {
...
it('rejects malformed theme discovery directories', function (Closure $arrange, string $message): void {
    $root = sys_get_temp_dir().'/liberu-themes-'.bin2hex(random_bytes(5));
    mkdir($root, 0777, true);
    $arrange($root);
    expect(fn (): array => new ThemeDiscovery([])->discover($root))->toThrow(InvalidTheme::class, $message);
})->with([
    'name collision' => [function (string $root): void {
...
it('rejects an application with no themes at all', function (): void {
    expect(fn (): array => new ThemeDiscovery([])->discover(sys_get_temp_dir().'/absent-'.bin2hex(random_bytes(5))))
...
it('finds Composer-installed themes with no tracked tree at all', function (): void {
...
it('counts a theme the tracked tree and Composer both name once', function (): void {
    $root = sys_get_temp_dir().'/liberu-themes-'.bin2hex(random_bytes(5));
    mkdir($root, 0777, true);
...
it('rejects two different packages claiming one theme name', function (): void {
    $root = sys_get_temp_dir().'/liberu-themes-'.bin2hex(random_bytes(5));
    mkdir($root, 0777, true);
    rename(coverageThemePackage('covered'), $root.'/covered');

    expect(fn (): array => new ThemeDiscovery([coverageThemePackage('covered')])->discover($root))
        ->toThrow(InvalidTheme::class, 'directory/name collision');
});

it('ignores tracked directories that do not contain a theme manifest', function (): void {
...
it('reports theme cache filesystem write and delete failures', function (): void {
...
it('rejects invalid theme manifests', function (array $changes, string $message, bool $asset = true): void {
    expect(fn (): ThemeManifest => ThemeManifest::fromFile(writeCoverageTheme($changes, $asset)))->toThrow(InvalidTheme::class, $message);
})->with([
